...
void mergeFileAtLine(const char *fileMerge1, const char *fileMerge2, const char *fileFinal, int lineMerge)
{
    ...
    else
    {
        ...
        close(pipefd[0]);
        history("merged file '%s' and '%s' to '%s' at line %d.", fileName, fileName2, fileName3, lineMerge);
        exit(0);
    }
...
void menuMerge()
{
    do
    {
        printMenuMerge();
        int choice_merge;
        if (scanf("%d", &choice_merge) != 1 || choice_merge < 1 || choice_merge > 3)
        {
            printf("Invalid input\n");
            continue;
        }
